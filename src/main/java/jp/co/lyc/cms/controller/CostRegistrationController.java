...
public class CostRegistrationController extends BaseController {
	...
	@ResponseBody
	public List<CostRegistrationModel> selectCostRegistration(
			@RequestBody CostRegistrationModel costRegistrationModel) {
	...
	public boolean insertCostRegistration(@RequestParam(value = "emp", required = false) String JSONEmp,
		...
		CostRegistrationModel costRegistrationModel = JSON.parseObject(jsonObject.toJSONString(),
				new TypeReference<CostRegistrationModel>() {
				});
		if (costRegistrationModel.getEmployeeNo() == null) {
			costRegistrationModel.setEmployeeNo(getSession().getAttribute("employeeNo").toString());
			costRegistrationModel.setEmployeeName(getSession().getAttribute("employeeName").toString());
		...
		if (!getFilename.equals("")) {
			try {
				...
				fileKey = fileKey.substring(0, fileKey.lastIndexOf("/") + 1) + costRegistrationModel.getEmployeeNo()
						+ "_" + costRegistrationModel.getEmployeeName() + "_"
	...
	public boolean updateCostRegistration(@RequestParam(value = "emp", required = false) String JSONEmp,
		...
		String getFilename = "";
		if (costFile == null && !costRegistrationModel.getCostClassificationCode()
		...
		if (costRegistrationModel.isChangeFile()) {
			// ファイル名に変更がある
			/*
			 * if(!costRegistrationModel.getCostClassificationCode().equals(
			 * costRegistrationModel.getOldCostClassificationCode())||
			 * !costRegistrationModel.getHappendDate().equals(costRegistrationModel.
			 * getOldHappendDate())){
			 */
			// 新しいファイルがない
			if (costFile == null) {
				if (!costRegistrationModel.getCostClassificationCode()
						.equals(costRegistrationModel.getOldCostClassificationCode())) {
					if (costRegistrationModel.getCostClassificationCode().equals("1")
							|| costRegistrationModel.getOldCostClassificationCode().equals("1")) {
						// 区分が変更された,旧ファイル削除
				...
				try {
					...
					try {
						...
						String filePath = getFilename.replaceAll("\\\\", "/");
						String fileKey = filePath.split("file/")[1];
						fileKey = fileKey.substring(0, fileKey.lastIndexOf("/") + 1)
								+ costRegistrationModel.getEmployeeNo() + "_" + costRegistrationModel.getEmployeeName()
								+ "_" + fileKey.substring(fileKey.lastIndexOf("/") + 1, fileKey.length());
						s3Model.setFileKey(fileKey);
						s3Model.setFilePath(filePath);
						s3Controller.uploadFile(s3Model);
					} catch (Exception e) {
	...
	public String upload(CostRegistrationModel costRegistrationModel, MultipartFile costFile) {
		...
		String realPath = new String(UPLOAD_PATH_PREFIX + "作業報告書フォルダ" + File.separator
				+ costRegistrationModel.getHappendDate().substring(0, 4) + File.separator
				+ costRegistrationModel.getHappendDate().substring(4, 6));
		...
		String newName = (costRegistrationModel.getCostClassificationCode().equals("0")
				? costRegistrationModel.getHappendDate().substring(0, 6)
				: costRegistrationModel.getHappendDate()) + costRegistrationModel.getCostClassificationName() + "."
				+ suffix;
	...
	private String getName(CostRegistrationModel costRegistrationModel, MultipartFile costFile) {
		...
		if (costFile != null) {
			...
			String newName = costRegistrationModel.getHappendDate().substring(4, 8)
					+ costRegistrationModel.getCostClassificationName() + "." + suffix;
			return realPath + File.separator + newName;
		}
	}
}
